...
namespace ALP_Revisi__2_
{
    public partial class RegisMember : Form
    {
        ...
        {
            SqlConnect = new MySqlConnection("server=localhost;uid=root;pwd=-;database=indramar_20232_dbd_6");
            SqlConnect.Open();
            SqlConnect.Close();
            ...
            DGV_RegisMember.DataSource = dtMember;
        ...
        private void bt_Registration_Click(object sender, EventArgs e)
        {
            ...
            if (tb_NamaRM.Text != "" && tb_NoTelpRM.Text != "" && tb_EmailRM.Text != "" && choose == true)
            {
                ...
                {
                    if (dtMember.Rows[i][2].ToString().ToLower() == tb_NamaRM.Text.ToLower() && dtMember.Rows[i][4].ToString() == tb_EmailRM.Text.ToLower())
                    {
                        exist = true;
